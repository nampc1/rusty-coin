        ...
        for cmd in &self.cmds {
            match cmd {
                ...
                Cmd::Op(op) => match op {
                    ...
                    OpCode::OpCheckSig => {
                        let Some(pub_key_bytes) = stack.pop() else {
                            return false;
                        };
                        let Some(raw_sig) = stack.pop() else {
                            return false;
                        };
                        ...
                        let Ok(signature) = Signature::parse_der(signature_bytes) else {
                            return false;
                        };
